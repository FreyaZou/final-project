...
fk_coord <- data.frame(lon = -73.778889, lat = 40.639722, name = 'JFK Airport')
la_guardia_coord <- data.frame(lon = -73.872611, lat = 40.77725,name = 'LGA Airport')
train_coord <- data.frame(lon = -73.993584, lat = 40.750580, name = 'train station')
bus_coord <- data.frame(lon = -73.9903, lat = 40.7569,name = 'bus station')

hot_location = data.frame(rbind(fk_coord,la_guardia_coord,train_coord,bus_coord))
...
foo %>%
  ...
  #add_markers(lat = ~hot_location$lat, lon=~hot_location$lon, text = ~hot_location$name) %>%
...
speed_df = train %>%
  group_by(vendor_id) %>%
  summarize(min = min(speed), max = max(speed), median = median(speed))
...
In order to check if the pick-up locaiton is associated with the average speed of each trip, we draw the following map with color changed by average speed. We find in general, trips start from airport travel in higher speed.
